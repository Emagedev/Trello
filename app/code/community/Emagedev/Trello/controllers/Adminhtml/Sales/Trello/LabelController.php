 ...
 */
class Emagedev_Trello_Adminhtml_Sales_Trello_LabelController extends Mage_Adminhtml_Controller_Action
{
    protected $_model = 'trello/label';

    /**
     * Init layout
     *
     * @return Emagedev_Trello_Adminhtml_Sales_Trello_LabelController
     */
    protected function _initAction()
    {
        $this->loadLayout()
            ->_setActiveMenu('sales/trello_label')
            ->_addBreadcrumb($this->__('Trello Label'), $this->__('Trello Label'));
        $this->_title($this->__('Trello Label'));
    ...
    public function editAction()
    {
        ...
        if ($modelId) {
            ...
            if ($model->getId()) {
                ...
                Mage::register('current_trello_label', $model);
            } else {
    ...
    public function fetchLabelsAction()
    {
        /** @var Emagedev_Trello_Helper_Data $dataHelper */
        $dataHelper = Mage::helper('trello');

        try {
            $dataHelper->updateLabels();
        } catch (Exception $e) {
            $this->_getSession()->addError($this->__('Unable To Fetch Labels'));
            Mage::logException($e);
            $dataHelper->log('Unable To Fetch Labels Due To Exception: ' . $e->getMessage(), Zend_Log::CRIT);

            return $this->_redirect('*/*/');
        }

        $this->_getSession()->addSuccess($this->__('Labels Fetched Successfully'));
